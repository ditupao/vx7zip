...
DWORD WINAPI GetFullPathName( LPCTSTR name, DWORD len, LPTSTR buffer, LPTSTR *lastpart ) { // FIXME
	...
	//	  lstrcpy(buffer,L"c:");
	  ...
	  begin[0]='/';
	//	begin[1]=':';
	//	char * cret = getcwd(begin, MAX_PATHNAME_LEN - 3);
	  char * cret = "/";
	  if (cret) {
...
{
  path = GetUnicodeString("tmp/"); // final '/' is needed
  return true;
}
